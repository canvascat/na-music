...
import type { Album, Lrc, Privilege, Song } from '@/api/types'
...
async function deleteExcessCache () {
  ...
  if (cacheLimit === false || tracksCacheBytes < (cacheLimit * (1 << 20))) return
  try {
...
export async function getTrackDetailFromCache (ids: (number | string)[]) {
  ...
  ids.forEach(id => {
    ...
    songs.push(one?.detail)
    privileges.push(one?.privileges)
  })
...
export async function cacheLyric (id: number | string, lyrics: Record<string, Lrc>) {
...
export async function getLyricFromCache (id: number | string) {
  ...
  return result.lyrics as { lrc: Lrc; tlyric: Lrc; }
}
...
export async function getAlbumFromCache (id: number | string) {
  ...
  if (!result) return undefined
  return result.album as { songs: Song[]; album: Album; }
}
